...
export const AI_FEATURES = {
  ...
  BIORHYTHM_MEMORY_DIGEST: "lite-flex", // 短期記憶: 前日の出来事を1日1回まとめる
  ...
  // 自分年表の月次ロールアップ。前月ぶんの日記をまとめて読み、「大きな出来事」と
  // 「そのころ世の中では」を選ぶ。**必ずローカルで回す。**
  // 理由は GROUNDING_RESEARCH と同じで、本人しか読まない私的な日記を過去数か月ぶん
  // まとめて**もう一度、大量に**外へ出す処理だから（日記本文の初回生成とは性質が違う）。
  // バックフィルでは全ユーザー × 全月ぶん走るので、総量から見ても自前が妥当。
