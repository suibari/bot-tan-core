...
「今日はガンダムの話で盛り上がって、あの世界観やモビルスーツの魅力をたっぷり語り合った」
という定期ポスト相当文で、2026-09-23に実API経路を試した。

- ローカルLLM抽出: `ガンダム` → `Mobile Suit Gundam`（シリーズ全体の言及）
- AnimeThemes: 1979年版のOP `Tobe! Gundam (Fly! Gundam)`
- Last.fm歌手補完: 池田鴻
- YouTube照合: `翔べ！ガンダム` / 池田鴻 - Topic（videoId `H1DelNpdvTU`）
- 安全・日本語ゲート: 通過
- 最終結果: `Tobe! Gundam` — 池田鴻、公式OPという関係を紹介文にも反映

作品を検出できない、AnimeThemesに登録がない、曲の言語が投稿言語と合わない、または安全確認を
通らない場合は、従来の気分タグ選曲へ戻る。Geminiフォールバックは追加していない。

## 重複防止と外部API監視

本番経路は投稿直前にDB予約を取る。定期ポスト同士、または同じDIDのDJリクエスト同士が並行しても、
同じ曲キー・YouTube videoIdを片方しか予約できない。通常予約は15分で失効し、外部投稿直前に
`publishing`へ昇格した予約は30日保護される。投稿先が一つでも成功すれば成功扱いで確定を試み、
確定だけが失敗しても保護は残す。全投稿先の失敗を確認できた場合だけ解放する。30日は選出時刻からの
ローリング窓で、境界時刻を含む。

Last.fm、AnimeThemes、YouTubeには共通15秒のタイムアウトを設定し、各実通信の成否と
`elapsed_ms`を記録する。検索語や投稿本文は遅延ログへ含めない。
