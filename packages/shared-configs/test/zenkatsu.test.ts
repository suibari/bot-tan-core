...
test("クールダウン日数はレアが上がるほど長い（排出率と逆相関）", () => {
  assert.ok(
...
test("出した札は rest 日ぶんクールダウンし、rest+1 日目に戻る", () => {
  const play = [{ volume: 1, id: 1, themeDate: "2026-09-19" }];
  // N は2日クールダウン。出した当日と翌日・翌々日は出せない。
...
test("被りは在庫になる（duplicate_count が生きる）", () => {
  // 3枚持っていれば3日連続で出せる。N は2日クールダウンなので4日目には1枚目が戻る。
