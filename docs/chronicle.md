...
年表の既読は `nagi.chronicle_read_years` に、**1アカウント・1年につき1行**保存する。
既存の認証付き `getPreferences` / `putPreferences` の `chronicleReadYears` で同期する。
要素は `{ year, revision }`。OAuthスコープの追加は不要。

`getChronicle` はその年の全項目と `year` / `revision` を返す。
`revision` はサーバーが年と内容から計算するSHA-256。項目IDでソートし、
項目ID・種別・日付・日英の見出しと本文・日記日付、ニュースのURI/CID、
カードの識別子・日英の名前とコメントを対象とする。
表示言語や取得順序では変わらず、項目の追加・変更・削除で変わる。
年単位の既読なので、互換用の `limit` パラメーターによる切り捨てはしない。

読んだ年の現在のハッシュと保存済みハッシュが違えば未読。
読み直すとその年の保存値を上書きする。過去の版は蓄積せず、4,096件という総量上限も設けない。
読み込んでいない年は既読にしない。初めから空の年は通知しないが、既読の年から全項目が
削除された場合はハッシュの変更で未読になる。

保存時には、そのアカウントの実際の年表をトランザクション内で取得し直してハッシュを検証する。
入力の年は2020年から現在年まで、かつそのアカウントの年表の起点以降に限定する。
任意のハッシュや更新前の古いハッシュは保存せず、応答で現在の既読を返す。
アカウント単位のロックで検証と上書きを直列化し、古い画面の再送による巻き戻りを防ぐ。
1リクエスト100年まで受け付けるが、総保存件数は年数だけで決まる。

クライアントは年ごとの既読と未送信分をキャッシュし、通信失敗時は次の閲覧・画面復帰で再送する。
従来のlocalStorageの項目別既読は、その年の現在の全項目が既読だった場合だけ移行する。
メニューバッジの取得は年表・カードゲーム・ラジオとも、起動／ログイン時と画面復帰時。
本人の閲覧・プレイ完了時にもその場で更新し、定期取得はしない。
全データ削除・退会ではサーバーの年別既読とクライアントのキャッシュを削除する。

反映順はDBスキーマ（通常の `drizzle-kit push`、手動適用用は
`0080_nagi_chronicle_read_years.sql`）、AppView、クライアント。
未デプロイだった項目別リビジョン集合のテーブル・APIは、この年別方式で置き換える。
