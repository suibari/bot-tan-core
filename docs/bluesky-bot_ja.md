...
12. **お絵描き機能**: botたんに絵をリクエストすると、クレヨンで描いた絵をリプライでプレゼントします
13. **バッジ（Bluesky Label）機能**: 特定の条件を満たすことで、Bluesky上のあなたのプロフィールに特別なバッジを付与します

本botのコミュニティに参加することで追加機能が利用できます。

- 🎮 **Discordコミュニティ**: [botたんDiscordサーバー](https://discord.gg/hshXWQEMgu) に参加し、Blueskyアカウントと連携すると、日記機能・応援機能・記念日機能・一年のまとめ機能・お絵描き機能とチーム全肯定バッジが利用できます。
- 💖 **サブスク（Pixiv Fanbox）**: [Fanbox](https://suibari.fanbox.cc/posts/10174305) でサポートすると、さらに会話機能・高品質AIリプライが利用できます。

以下に各ティアで使用できる機能をまとめます。

| ティア | 定型文リプライ | AI生成リプライ | 占い | リプ頻度調整 | 会話機能 | 分析機能 | 応援機能 | DJ機能 | 日記機能 | 記念日機能 | 一年のまとめ | お絵描き | バッジ機能※1 |
| :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
| 通常フォロワー | ✓ | ※2 | ✓ | ✓ | | ✓ | | ✓ | | | | | ✓ |
| Discordメンバー | ✓ | ※2 | ✓ | ✓ | | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ |
| サブスクメンバー | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ |
...
botたんに絵をリクエストすると、クレヨンで描いた絵をプレゼントします。
お絵描きは **1日1回**（日本時間の日付で区切ります）までです。

1. "使用方法"に従い、本botからフォローされた状態となる
2. 本botに対しメンションまたはリプライで、描いてほしいものを添えて絵をお願いする（例:「botたん、猫の絵を描いて！」「Bot-tan, draw me a cat」）
   決まった合言葉はありません。お願いかどうかはbotたんが文面から判断します。
3. 本botが絵を描き、画像付きでリプライします。絵にはbotたんが一緒に描かれます。

※性的・暴力的な内容、実在の人物、既存の作品のキャラクターは描けません。
※混み合っているときや、うまく描けなかったときは描かずにお知らせします。うまく描けなかった場合、その日の1回は消費されません。

Nagiでは全ユーザーがお絵描き機能を利用できます。Blueskyと同じくbotたんにお願いすると描いてくれるほか、とても嬉しいことがあったときや、ひどく落ち込んでいるときに、botたんが自分の判断で絵を描いてプレゼントすることもあります。Nagiでのお絵描きは、お願いとプレゼントを合わせて1日1回までです（Blueskyとは別に数えます）。
...
本botが収集した情報は、リプライ生成以外の目的では第三者と共有されません。AI生成の本文は自宅のローカルモデルで作成します。最新情報の確認が必要な場合だけGoogle GeminiのGroundingを使用しますが、Google LLCへ送るのはローカルモデルが匿名化した検索語と対象URLに限り、元投稿、会話履歴、DID、botのシステムプロンプトは送信しません。

### 年齢制限
Groundingおよび画像生成でGoogle Gemini APIを使用する場合は、同APIの利用規約が適用されます。

**18歳未満の方は、AIを用いた機能の利用をお控えください。**

定型文リプライの分類と本文生成は自宅のローカルモデルで行います。

### 地域制限
Google GeminiのGroundingまたは画像生成を伴う機能には、同APIの地域要件が適用されます：
